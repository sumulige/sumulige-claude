...
function parseSourcesYaml(content) {
  try {
    ...
    log(`Warning: Failed to parse sources.yaml (${e.message})`, 'yellow');
    return { skills: [] };
  }
}
  ...
  'marketplace:list': () => {
    ...
    if (fs.existsSync(SOURCES_FILE)) {
      try {
        ...
        sources = parseSourcesYaml(content);
      } catch (e) {
...
// Backward compatibility: legacy tolerant parser (line-based, forgiving indentation)
function parseSimpleYaml(content) {
  ...
  content.split('\n').forEach(line => {
    ...
    if (trimmed.startsWith('version:')) {
      ...
      result.version = Number.isNaN(num) ? result.version : num;
      return;
    }
...
exports.parseSourcesYaml = parseSourcesYaml;
