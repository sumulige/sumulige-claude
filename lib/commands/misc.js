...
function getTodoManagerPath() {
  ...
  const localPath = path.join(cwd, '.claude/hooks/todo-manager.cjs');
  if (fs.existsSync(localPath)) return localPath;

  const globalPath = path.join(os.homedir(), '.claude/hooks/todo-manager.cjs');
...
/**
 * 获取 todo-manager 运行提示
 */
function getTodoManagerHint() {
  const todoPath = getTodoManagerPath();
  if (todoPath) {
    return `Run: node "${todoPath}" --force`;
  }
  return "Run: smc init (to setup todo system)";
}
...
const commands = {
  ...
  agent: async (...args) => {
    ...
    const { buildExecutionPrompt } = require("../agent-orchestrator/execution-prompts");
    ...
    const {
      ...
      refreshTodoIndex,
      TODOS_DIR
    } = require("../agent-orchestrator/todo-bridge");

    // Parse options
    const options = {
      mode: process.env.SMC_AGENT_MODE || 'plan',
      runner: process.env.SMC_AGENT_RUNNER || 'auto',
      sandbox: process.env.SMC_AGENT_SANDBOX || '',
      fullAuto: process.env.SMC_AGENT_FULL_AUTO ? process.env.SMC_AGENT_FULL_AUTO === '1' : undefined,
      permissionMode: process.env.SMC_AGENT_PERMISSION_MODE || '',
      dryRun: false,
      verbose: false,
      list: false,
      createTodo: false,
    ...
    for (let i = 0; i < args.length; i += 1) {
      ...
      else if (arg === "--run" || arg === "--execute") options.mode = "run";
      else if (arg === "--plan") options.mode = "plan";
      else if (arg === "--no-skills") options.noSkills = true;
      else if (arg === "--runner" && args[i + 1]) options.runner = args[++i];
      else if (arg.startsWith("--runner=")) options.runner = arg.split("=")[1];
      else if (arg === "--sandbox" && args[i + 1]) options.sandbox = args[++i];
      ...
      else if (arg === "--permission-mode" && args[i + 1]) options.permissionMode = args[++i];
    ...
    if (options.list || !task) {
      ...
      console.log('  smc agent "实现用户登录" --run');
      console.log('  smc agent "Review code" --plan');
      console.log('  smc agent "添加支付功能" --create-todo');
      console.log("");
      console.log("Options:");
      console.log("  --plan             Default mode (no code changes)");
      console.log("  --run              Execute implementation");
      console.log("  --dry-run          Print prompt only");
      console.log("  --runner <name>    codex|claude|gemini|auto");
      console.log("  --sandbox <mode>   read-only|workspace-write|danger-full-access (codex)");
      console.log("  --full-auto        Enable codex full-auto (run mode)");
      console.log("  --no-skills        Disable auto skill activation");
      console.log("  --verbose, -v      Show detailed output");
      console.log("  --list, -l         List available agents");
      console.log("  --create-todo, -t  Create todo files from tasks (legacy)");
      console.log("");
      return;
    }

    // Legacy create-todo flow (kept for compatibility)
    if (options.createTodo) {
      try {
        ...
        const tasks = todoBridge.extractTasksFromInstruction(result.execution.instruction);
        if (tasks.length > 0) {
          console.log("📁 创建任务文件...");
    ...
    const skills = options.noSkills ? [] : autoActivateSkills({ task, projectDir });
    const routed = routePrompts({ task, kind: 'agent' });
    ...
    if (!runner) {
      console.log("⚠️  未检测到可用的 CLI Runner (codex/claude/gemini)");
      console.log("");
      console.log(prompt);
      return;
    }

    const sandbox = options.sandbox || (options.mode === 'plan' ? 'read-only' : 'workspace-write');
    const fullAuto = typeof options.fullAuto === 'boolean' ? options.fullAuto : options.mode === 'run';
    const permissionMode = options.permissionMode || (options.mode === 'plan' ? 'plan' : 'acceptEdits');

    if (options.verbose) {
      console.log(`Runner: ${runner}`);
      console.log(`Sandbox: ${sandbox}`);
      console.log(`Full-auto: ${fullAuto ? "yes" : "no"}`);
      console.log("");
    }
    ...
    if (routed.flags.globalUI) {
      ...
      if (!uipro.ok && uipro.reason === 'missing') {
        console.log(`⚠️  未检测到 uipro，可手动执行: uipro init --ai ${runner}`);
      }
    }

    const findTodoByTitle = (dir, title) => {
    ...
    const locateTodo = (title) => {
      const slug = slugify(title);
      const candidates = [
        { status: 'active', path: path.join(TODOS_DIR, 'active', `${slug}.md`) },
        { status: 'backlog', path: path.join(TODOS_DIR, 'backlog', `${slug}.md`) },
        { status: 'completed', path: path.join(TODOS_DIR, 'completed', `${slug}.md`) }
      ];
      for (const c of candidates) {
        if (fs.existsSync(c.path)) {
          return { status: c.status, filepath: c.path, filename: path.basename(c.path) };
        }
      }
      const byTitle = findTodoByTitle(path.join(TODOS_DIR, 'active'), title)
        || findTodoByTitle(path.join(TODOS_DIR, 'backlog'), title)
        || findTodoByTitle(path.join(TODOS_DIR, 'completed'), title);
      if (byTitle) {
        const status = byTitle.filepath.includes(`${path.sep}completed${path.sep}`) ? 'completed'
          : byTitle.filepath.includes(`${path.sep}backlog${path.sep}`) ? 'backlog'
          : 'active';
        return { status, filepath: byTitle.filepath, filename: byTitle.filename };
      }
      return null;
    };

    const appendOutputToTodo = (filepath, output) => {
      if (!output) return;
    ...
    try {
      ...
      if (output) {
        let todo = locateTodo(task);
        if (!todo) {
          const created = createSingleTodo({
            agent: 'builder',
            title: task,
            description: task,
            subtasks: [],
          ...
          if (created?.filepath) {
            todo = { status: 'active', filepath: created.filepath, filename: created.filename };
          }
        }

        if (todo?.filepath) {
          appendOutputToTodo(todo.filepath, output);
          if (options.mode === 'run' && todo.status !== 'completed') {
            updateTodoStatus(todo.filepath, 'completed');
          }
  ...
  status: () => {
    ...
    if (fs.existsSync(todosIndex)) {
      ...
      if (!countsFound) {
        ...
        console.log(`  ${getTodoManagerHint()}`);
      }
