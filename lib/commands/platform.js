...
const commands = {
  ...
  "platform:convert": (...args) => {
    ...
    if (fromConfig) {
      ...
      let unified;
      if (from === "claude") {
        unified = configConverter.fromClaude(fromConfig);
      } else {
        unified = configConverter.fromCodex(fromConfig);
      }

      let toConfig;
      if (to === "claude") {
        toConfig = configConverter.toClaude(unified);
      } else {
        toConfig = configConverter.toCodex(unified);
      }

      if (dryRun) {
        log("   Preview:", "gray");
        const content = to === "claude"
          ? configConverter.stringifyClaude(toConfig)
          : configConverter.stringifyCodex(toConfig);
    ...
    if (fromInstructions) {
      ...
      if (from === "claude" && to === "codex") {
        toContent = instructionConverter.claudeToAgents(fromInstructions.content, { projectName });
      } else if (from === "codex" && to === "claude") {
        toContent = instructionConverter.agentsToClaude(fromInstructions.content, { projectName });
      }
