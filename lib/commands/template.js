...
const commands = {
  ...
  template: (...args) => {
    ...
    if (showHelp) {
      console.log("");
      console.log("📋 smc template - Deploy AI CLI project template");
      console.log("");
      console.log("USAGE:");
      ...
      console.log("  --codex     Deploy Codex CLI template only");
      console.log("  --all       Deploy both Claude Code and Codex CLI templates");
      console.log("  --help, -h  Show this help message");
      console.log("");
      console.log("DEFAULT BEHAVIOR:");
      console.log("  Deploy Claude Code template only.");
      console.log("  Backup existing files before overwriting.");
      console.log("  Backups stored in: .claude/backup/");
      console.log("");
      console.log("EXAMPLES:");
      console.log(
        "  smc template                 # Deploy Claude Code (with backup)",
      );
      console.log("  smc template --codex         # Deploy Codex CLI only");
      console.log("  smc template --all           # Deploy both platforms");
    ...
    // Handle Codex-only deployment
    if (deployCodex && !deployAll) {
      commands.deployCodexTemplate(targetPath, copyMode);
      return;
    }
...
`;
    ...
    console.log("");
    console.log("✅ Claude Code template initialization complete!");
    console.log("");
    ...
    // Deploy Codex template if --all
    if (deployAll) {
      console.log("");
      commands.deployCodexTemplate(targetDir, copyMode);
    }
    ...
    if (deployAll) {
      console.log("   3. Run: codex   # Start Codex CLI");
    }
  ...
  // -------------------------------------------------------------------------
  // deployCodexTemplate - Deploy Codex CLI template
  // -------------------------------------------------------------------------
  deployCodexTemplate: (targetPath, copyMode = CopyMode.BACKUP) => {
    const targetDir = targetPath || process.cwd();
    const backupDir = path.join(targetDir, ".codex", "backup");

    console.log("🦊 Initializing Codex CLI project template...");
    console.log("   Target:", targetDir);
    console.log("");
    ...
    // Copy config.toml
    const configSrc = path.join(TEMPLATE_DIR, ".codex", "config.toml");
    const configDest = path.join(codexDir, "config.toml");
    if (fs.existsSync(configSrc)) {
      copySingleFile(configSrc, configDest, copyMode, backupDir, ".codex/config.toml");
    }

    // Copy README.md
    const readmeSrc = path.join(TEMPLATE_DIR, ".codex", "README.md");
    const readmeDest = path.join(codexDir, "README.md");
    if (fs.existsSync(readmeSrc)) {
      copySingleFile(readmeSrc, readmeDest, copyMode, backupDir, ".codex/README.md");
    }

    // Copy AGENTS.md to project root
    const agentsSrc = path.join(TEMPLATE_DIR, "AGENTS.md");
    const agentsDest = path.join(targetDir, "AGENTS.md");
    if (fs.existsSync(agentsSrc)) {
      copySingleFile(agentsSrc, agentsDest, copyMode, backupDir, "AGENTS.md");
    }

    console.log("");
    console.log("✅ Codex CLI template initialization complete!");
    console.log("");
    console.log("📦 What was included:");
    console.log("   • .codex/config.toml - Codex configuration");
    console.log("   • .codex/README.md - Codex setup guide");
    console.log("   • AGENTS.md - Project instructions for Codex");
    console.log("");
    console.log("Next steps:");
    console.log("   1. Edit AGENTS.md with your project info");
    console.log("   2. Customize .codex/config.toml if needed");
    console.log("   3. Run: codex  # Start Codex CLI");
    console.log("");
  },
  ...
  kickoff: async (...args) => {
    if (args[0] === 'agent') {
      ...
      if (!task) {
        console.log('Usage: smc kickoff agent "<task>"');
        console.log("");
        console.log('Example: smc kickoff agent "构建多端聊天聚合平台"');
        console.log("");
        console.log("Options:");
        ...
        console.log("  --full-auto        Enable codex full-auto");
        console.log("  --no-skills        Disable auto skill activation");
        console.log("");
        return;
      }

      console.log("🚀 Kickoff Agent");
      console.log("");
      ...
      const routed = routePrompts({ task, kind: 'kickoff' });
      ...
      try {
        ...
        if (output) {
          ...
          if (tasks.length > 0) {
            ...
            console.log("⚠️  未解析到子任务，未创建 todos");
          }
        }
      } catch (error) {
